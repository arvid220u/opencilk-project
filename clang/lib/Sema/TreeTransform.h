...
TreeTransform<Derived>::TransformCilkForRangeStmt(CilkForRangeStmt *S) {
  ...
  StmtResult CilkForRange = getDerived().RebuildCilkForRangeStmt(ForRange.get());
  if (CilkForRange.isInvalid())
    return StmtError();

  return getDerive().FinishCilkForRangeStmt(CilkForRange.get(), cast<CXXForRangeStmt>(ForRange.get())->getBody())
}
